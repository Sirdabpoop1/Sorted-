...
hcc_worksheet = sh.worksheet("ACC")
...
for i, row in enumerate(form, start = 1):
    ...
    if grade and not done:
        ...
        del disec_taken[0]
        hcc_taken = safe_read(hcc_worksheet.col_values, 4)
        del hcc_taken[0]
        ...
        #         break
        #     elif choice == "HCC":
        #         prefered_cell = safe_read(hcc_worksheet.find, del_preference[a])
        #         hcc_prefered_point = safe_read(hcc_worksheet.cell, prefered_cell.row, 3).value
        ...
        # else:
        #     cell = safe_read(hcc_worksheet.find, str(ideal_coun_points/2))
        #     safe_update(hcc_worksheet.update_cell, cell.row, 2, name)
        #     safe_update(hcc_worksheet.update_cell, cell.row, 4, "Taken")
        #     delegation = safe_read(hcc_worksheet.cell, cell.row, 1).value
